...
clf = svm.SVC(kernel='linear')  # instantiation of the classifier as an SVM
...
def trainAndTestModel():
    ...
    clf.fit(feat_train, lab_train)  # This line trains the classifier
    # This line runs a prediction on the testing set
    prediction = clf.predict(feat_test)
    print("Accuracy:", metrics.accuracy_score(lab_test, prediction))
...
class Testing(unittest.TestCase):
    # Run 'python -m unittest svm.Testing' from the command line to run the tests
    def setUp(self):
        wipeVariables()
        return super().setUp()
