...
import pickle  # Importing the model persistence code
import numpy as np
import csv
import copy
import unittest
...
def trainAndTestModel():
    ...
    scaler.fit(feat_train)  # Normalisation of input
    feat_train = scaler.transform(feat_train)
    feat_test = scaler.transform(feat_test)
    mlp.fit(feat_train, lab_train)  # This line trains the classifier
...
def predict():
    """ This method will take the input data from the input csv file and query the classifier on the data, loading and saving are both handled by this method """
    loadModel()
    takeInput()
    scaler = StandardScaler()  # Normalising the input
...
class Testing(unittest.TestCase):
    # Run 'python -m unittest ann.Testing' from the command line to run the tests
    def setUp(self):
        wipeVariables()
        return super().setUp()
